...
process_day() {
  local day="$1"
  local day_dir="${WATCH_ROOT}/${day}"
  local day_output_dir="${OUTPUT_DIR}/${day}"
  local processed_state_file="${STATE_DIR}/${day}.processed"
  local uploaded_state_file="${STATE_DIR}/${day}.uploaded"
  local video_path
  local -a mp4_files

  if [[ ! -d "${day_dir}" ]]; then
    echo "等待当天目录创建: ${day_dir}"
    return 0
  fi
...
process_all_days() {
  ...
  mapfile -t day_dirs < <(find "${WATCH_ROOT}" -mindepth 1 -maxdepth 1 -type d | sort)

  if [[ "${#day_dirs[@]}" -eq 0 ]]; then
    echo "监听根目录下暂无任何子目录: ${WATCH_ROOT}"
    return 0
  fi
